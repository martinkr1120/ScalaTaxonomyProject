...
class GraphFramesSearchSpec extends FlatSpec with Matchers {
  ...
  val graph = GraphFrame(veDF, edParentDF).persist(StorageLevel.MEMORY_ONLY).cache()

  behavior of "GraphFramesSearch getPathToRoot"
  it should "work for 12429 to root" in {
    GraphFramesSearch.getPathToRoot(graph,12429,List(12429)) shouldBe List(1, 10239, 12429)
  }
  ...
  it should "work for vid 1 is a subtree of 12429" in {
    GraphFramesSearch.isSubTree(graph,1,12429) shouldBe false
  }
  it should "work for vid 12429 is a subtree of 1" in {
    GraphFramesSearch.isSubTree(graph,12429,1) shouldBe true
  }
  ...
  it should "work for Homo sapiens,whose id is 9606" in {
    GraphFramesSearch.getSiblings(graph,10239).toSet shouldBe Set(12884, 12908, 28384, 131567)
  }
  ...
  it should "work for root, whoes id is 1" in {
    GraphFramesSearch.getChildren(graph,1).toSet shouldBe Set(10239, 12884, 12908, 28384, 131567)
  }
  it should "work for 10239" in {
    GraphFramesSearch.getChildren(graph,10239).toSet shouldBe
